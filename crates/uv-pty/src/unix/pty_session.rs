...
/// A high-level PTY session that manages an interactive shell process.
///
/// Provides methods to send input, wait for patterns in output, and
/// interact with the process in a terminal-like fashion.
...
impl PtySession {
    /// Constructs a new session by spawning the given command in a PTY.
    ...
    ///
    /// As stdin of the process is most likely buffered, call `flush()` after
    /// `send()` to make the process actually see the input.
    ///
    /// Returns the number of written bytes.
    pub fn send<B: AsRef<[u8]>>(&mut self, s: B) -> io::Result<usize> {
        // Sleep briefly to allow the shell to process previous input.
        std::thread::sleep(Duration::from_millis(50));
        self.process_stdin.write(s.as_ref())
    }

    /// Sends a string and a newline to the process. This is guaranteed to be flushed.
    ///
    /// Returns the number of written bytes.
    ...
    /// Flush all pending bytes to the process stdin.
    ...
    ///
    /// This puts the current terminal into raw mode and forwards all input from stdin
    /// to the process and all output from the process to stdout.
    /// Blocks until the process exits.
    ///
    /// If `wait_until` is `Some`, output is buffered until the pattern is found,
    /// then the remaining buffered output is flushed and interactive mode begins.
    pub fn interact(&mut self, wait_until: Option<&str>) -> io::Result<Option<i32>> {
        ...
        let exit_status = loop {
            ...
            for signal in signals.pending() {
                match signal {
                    SIGWINCH => {
                        // SAFETY: Querying and setting window size via ioctl is safe.
            ...
            if pattern_start.elapsed() > pattern_timeout && !write_stdout {
                io::stdout().write_all(
                    format!(
                        "warning: Did not detect successful shell activation within {} second(s).\n\r",
            ...
            if let Err(error) = res {
                if error == Errno::EINTR {
                    // EINTR means we got interrupted by a signal (e.g., SIGWINCH).
                    continue;
                }
