...
async fn test_ssl_cert_file_invalid_trust_anchor_returns_error() -> Result<()> {
    ...
    async_with_vars(vars, async move {
        ...
        with_settings!({
            ...
            assert_snapshot!(display, @r#"
failed to build HTTP client
Caused by: certificate in `[TMP]/ca.pem` (from `SSL_CERT_FILE`) could not be used as a trust anchor on certificate `CN=uv-test-ca, O=Astral Software Inc.`: invalid certificate extension value; duplicate extensions: `2.5.29.19`
"#);
        });
    })
    .await;
