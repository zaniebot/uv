...
pub struct InitArgs {
    ...
    /// under `tool.uv.sources`. This option is only applicable when the `from-project` argument is used.
    #[arg(long)]
    pub raw_sources: bool,

    /// Skips syncing the virtual environment after re-locking the project.
    ///
    /// This option is only applicable when the `from-project` argument is used.
    #[arg(long)]
    pub no_sync: bool,
}
