...
unsafe extern "system" {
    /// SAFETY:
    /// - `processhandle` must be a valid process handle.
    /// - The handle must grant `PROCESS_SUSPEND_RESUME` access.
    /// - The target process should be in a suspended state when used in this module.
...
fn resume_process(process: HANDLE) -> std::io::Result<()> {
    // SAFETY:
    // - `process` comes from `std::process::Child` and is a valid process handle.
    // - The child is created with `CREATE_SUSPENDED`, so resuming is expected.
    // - `Child` process handles are opened with sufficient rights for resume.
