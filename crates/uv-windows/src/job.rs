...
///
/// Designed to work without `std::fmt` - consumers can extract the raw error
/// code and format it according to their needs.
...
/// // When `job` is dropped (e.g., parent exits), child is terminated
/// ```
pub struct Job {
    handle: HANDLE,
}
