...
pub fn install_ctrl_handler() -> Result<(), CtrlHandlerError> {
    ...
    // SAFETY: We're registering a valid handler function. The `Some(Some(...))` pattern
    // is required by the windows crate 0.59.0 API.
    unsafe { SetConsoleCtrlHandler(Some(Some(handler)), true) }
        .map_err(|e| CtrlHandlerError(e.code().0))
}
