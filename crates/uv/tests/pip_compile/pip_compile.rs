...
/// Version-valued `in` and `not in` markers use substring semantics.
#[test]
fn compile_version_marker_in_substring() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(indoc! {r#"
        iniconfig==1.1.1 ; python_version in "3.12.1"
        tomli==2.0.1 ; python_version not in "3.12.1"
    "#})?;
