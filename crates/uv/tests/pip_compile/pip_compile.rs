...
#[cfg(feature = "test-universal")]
#[test]
fn universal_editable_marker() -> Result<()> {
    let context = uv_test::test_context!("3.12");

    let editable = context.temp_dir.child("editable");
    editable.create_dir_all()?;
    editable.child("pyproject.toml").write_str(indoc! {r#"
        [project]
        name = "editable"
        version = "0.1.0"
    "#})?;
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("-e ./editable ; sys_platform == 'linux'")?;
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --universal --no-index --offline
    -e ./editable ; sys_platform == 'linux'
        # via -r requirements.in
