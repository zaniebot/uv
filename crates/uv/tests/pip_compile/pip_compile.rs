...
/// Overrides and constraints must not activate a dependency guarded by either of two extras.
#[test]
fn dont_add_override_for_non_activated_extra_disjunction() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    scenario.packages.insert(
        ...
        Package {
            versions: BTreeMap::from([(
                ...
                PackageMetadata {
                    requires: vec![Requirement::from_str(
                        "child==1.0.0 ; extra == 'foo' or extra == 'bar'",
                    )?],
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("parent==1.0.0")?;
    let overrides_txt = context.temp_dir.child("overrides.txt");
    overrides_txt.write_str("child==1.0.0")?;
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --constraint overrides.txt
    parent==1.0.0
        # via -r requirements.in
