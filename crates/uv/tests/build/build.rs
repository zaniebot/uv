...
fn build_entry_point_name_injection() -> Result<()> {
    ...
    error: Failed to build `[TEMP_DIR]/`
      Caused by: Invalid project metadata
      Caused by: Entrypoint name "plugin/n/n[console_scripts]/nowned" must not contain a newline
    "#);
...
fn build_entry_point_object_injection() -> Result<()> {
    ...
    error: Failed to build `[TEMP_DIR]/`
      Caused by: Invalid project metadata
      Caused by: Entrypoint object reference "project:plugin/n/n[console_scripts]/nowned = project:main" must not contain a newline
    "#);
