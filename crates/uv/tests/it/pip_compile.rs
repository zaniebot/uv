...
fn invalid_platform() -> Result<()> {
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because open3d<=0.15.2 has no wheels with a matching Python ABI tag and open3d>=0.16.0 has no wheels with a matching platform tag, we can conclude that open3d<0.16.1 cannot be used.
