...
/// Use an existing resolution for `black==23.10.1`, with stale versions of `click` and `pathspec`.
/// With `--upgrade --upgrade-strategy only-if-needed`, `black` is already at the requested
/// version (pinned to `23.10.1`), so nothing changes. The transitive dependencies retain their
/// stale versions because the strategy only upgrades direct dependencies.
#[test]
fn upgrade_strategy_only_if_needed() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("black==23.10.1")?;
    ...
    // With `--upgrade --upgrade-strategy only-if-needed`, only the direct dependency `black`
    // (pinned at ==23.10.1) is considered for upgrade. Transitive deps keep their lockfile
    // versions.
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .arg("--output-file")
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --output-file requirements.txt --upgrade-strategy only-if-needed
    black==23.10.1
        # via -r requirements.in
    click==8.1.2
        # via black
...
/// Use an existing resolution for `black`, with stale versions of transitive dependencies.
/// With `--upgrade --upgrade-strategy only-if-needed` and an unpinned direct dependency,
/// the direct dependency (`black`) should be upgraded to the latest version, while transitive
/// dependencies should retain their lockfile versions.
#[test]
fn upgrade_strategy_only_if_needed_unpinned() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    ...
    // With `--upgrade --upgrade-strategy only-if-needed`, `black` (unpinned) gets upgraded to
    // the latest, while transitive deps retain lockfile versions.
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .arg("--output-file")
        .arg("requirements.txt")
...
/// With `--upgrade --upgrade-strategy eager` (the default), both direct and transitive
/// dependencies should be upgraded. This test confirms that `--upgrade-strategy eager` behaves
/// identically to `--upgrade` alone.
#[test]
fn upgrade_strategy_eager() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("black==23.10.1")?;
    ...
    // With `--upgrade --upgrade-strategy eager`, all packages are upgraded (same as `--upgrade`).
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .arg("--output-file")
        .arg("requirements.txt")
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --output-file requirements.txt --upgrade-strategy eager
    black==23.10.1
        # via -r requirements.in
    click==8.1.7
        # via black
