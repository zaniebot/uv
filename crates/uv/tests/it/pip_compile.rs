...
fn compile_requirements_in() -> Result<()> {
    ...
    requirements_in.write_str("swift-finch==3.7.0")?;
...
/// Resolve a specific version of `swift-finch` from a `requirements.in` file with a `--annotation-style=line` flag.
#[test]
fn compile_requirements_in_annotation_line() -> Result<()> {
    ...
    loud-warbler==3.6         # via swift-finch
    silly-wren==1.3.1         # via swift-finch
    swift-finch==3.7.0        # via -r requirements.in
...
/// Resolve a specific version of `swift-finch` from a `requirements.in` file on stdin
...
/// Resolve a specific version of `swift-finch` from a `pyproject.toml` file.
#[test]
...
fn compile_pyproject_toml_eager_validation() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let server = uv_test::packse::PackseServer::for_packages("general.toml");
    let pyproject_toml = context.temp_dir.child("pyproject.toml");
    ...
    ----- stderr -----
      ...
      ╰─▶ Because there is no version of anyio==4.7.0 and project depends on anyio==4.7.0, we can conclude that your requirements are unsatisfiable.
    ");
...
fn compile_constraints_markers() -> Result<()> {
    ...
    constraints_txt.write_str("silly-wren==1.2.0;python_version<='3.7'")?;
    constraints_txt.write_str("silly-wren==1.3.0;python_version>'3.7'")?;
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because there is no version of silly-wren==1.3.0 and all versions of swift-finch depend on silly-wren==1.3.0, we can conclude that all versions of swift-finch cannot be used.
          And because you require swift-finch, we can conclude that your requirements are unsatisfiable.
    "
    );
...
fn compile_constraint_extra() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("golden-crane")?;

    // Constrain a transitive dependency based on the Python version
    let constraints_txt = context.temp_dir.child("constraints.txt");
    constraints_txt.write_str("golden-crane[egg]<24.3.0")?;
...
/// Resolve a specific version of bold-eagle at Python 3.12 with `--annotation-style=line`.
#[test]
fn compile_python_312_annotation_line() -> Result<()> {
    ...
    bold-eagle==23.10.1       # via -r requirements.in
    bright-feather==0.12.1    # via bold-eagle
    sharp-talon==4.2.0        # via bold-eagle
...
fn compile_fallback_interpreter_broken_in_path() -> Result<()> {
    ...
    let context = uv_test::test_context!("3.10").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("bold-eagle==23.10.1")?;
...
/// Resolve a specific version of bold-eagle at Python 3.12 without deps.
#[test]
...
/// Resolve a specific version of bold-eagle at Python 3.7.
#[test]
fn compile_python_37() -> Result<()> {
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because the requested Python version (>=3.7) does not satisfy Python>=3.12 and bold-eagle==23.10.1 depends on Python>=3.12, we can conclude that bold-eagle==23.10.1 cannot be used.
          And because you require bold-eagle==23.10.1, we can conclude that your requirements are unsatisfiable.

          hint: The `--python-version` value (>=3.7) includes Python versions that are not supported by your dependencies (e.g., bold-eagle==23.10.1 only supports >=3.12). Consider using a higher `--python-version` value.
    ");
...
/// Resolve a specific version of bold-eagle against an invalid Python version.
#[test]
...
fn requirement_override_prerelease() -> Result<()> {
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("golden-crane<2.0.0rc4")?;

    let overrides_txt = context.temp_dir.child("overrides.txt");
    overrides_txt.write_str("golden-crane<2.0.1,!=2.0.0")?;
...
fn compile_exclude_newer() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("spotted-owl")?;
    ...
    ----- stderr -----
    warning: spotted_owl-4.66.2.tar.gz is missing an upload date, but user provided: 2022-04-04T12:00:00Z
    warning: spotted_owl-4.66.2-py3-none-any.whl is missing an upload date, but user provided: 2022-04-04T12:00:00Z
    warning: spotted_owl-4.64.1.tar.gz is missing an upload date, but user provided: 2022-04-04T12:00:00Z
    warning: spotted_owl-4.64.1-py3-none-any.whl is missing an upload date, but user provided: 2022-04-04T12:00:00Z
    warning: spotted_owl-4.64.0.tar.gz is missing an upload date, but user provided: 2022-04-04T12:00:00Z
    warning: spotted_owl-4.64.0-py3-none-any.whl is missing an upload date, but user provided: 2022-04-04T12:00:00Z
      × No solution found when resolving dependencies:
    ...
    ----- stderr -----
    warning: spotted_owl-4.66.2.tar.gz is missing an upload date, but user provided: 2022-04-05T05:00:00Z
    warning: spotted_owl-4.66.2-py3-none-any.whl is missing an upload date, but user provided: 2022-04-05T05:00:00Z
    warning: spotted_owl-4.64.1.tar.gz is missing an upload date, but user provided: 2022-04-05T05:00:00Z
    warning: spotted_owl-4.64.1-py3-none-any.whl is missing an upload date, but user provided: 2022-04-05T05:00:00Z
    warning: spotted_owl-4.64.0.tar.gz is missing an upload date, but user provided: 2022-04-05T05:00:00Z
    warning: spotted_owl-4.64.0-py3-none-any.whl is missing an upload date, but user provided: 2022-04-05T05:00:00Z
      × No solution found when resolving dependencies:
      ╰─▶ Because there are no versions of spotted-owl and you require spotted-owl, we can conclude that your requirements are unsatisfiable.
    "
    );
...
fn compile_exclude_newer_package() -> Result<()> {
    ...
    // spotted-owl 4.64.0 was released on 2022-04-04, 4.64.1 on 2022-09-03
    ...
    // Test override: allow spotted-owl to use newer versions while keeping requests pinned
    uv_snapshot!(context
        .pip_compile()
        .env_remove(EnvVars::UV_EXCLUDE_NEWER)
        .arg("--index-url")
        ...
        .arg("spotted-owl=2022-09-04T00:00:00Z"), @"
    success: false
    exit_code: 1
    ----- stdout -----

    ----- stderr -----
    warning: spotted_owl-4.66.2.tar.gz is missing an upload date, but user provided: 2022-09-04T00:00:00Z
    warning: spotted_owl-4.66.2-py3-none-any.whl is missing an upload date, but user provided: 2022-09-04T00:00:00Z
    warning: spotted_owl-4.64.1.tar.gz is missing an upload date, but user provided: 2022-09-04T00:00:00Z
    warning: spotted_owl-4.64.1-py3-none-any.whl is missing an upload date, but user provided: 2022-09-04T00:00:00Z
    warning: spotted_owl-4.64.0.tar.gz is missing an upload date, but user provided: 2022-09-04T00:00:00Z
    warning: spotted_owl-4.64.0-py3-none-any.whl is missing an upload date, but user provided: 2022-09-04T00:00:00Z
      × No solution found when resolving dependencies:
      ╰─▶ Because there are no versions of spotted-owl and you require spotted-owl, we can conclude that your requirements are unsatisfiable.
    "
    );
...
fn compile_exclude_newer_package_errors() -> Result<()> {
    ...
    ----- stderr -----
    error: invalid value 'spotted-owl' for '--exclude-newer-package <EXCLUDE_NEWER_PACKAGE>': Invalid `exclude-newer-package` value `spotted-owl`: expected format `PACKAGE=DATE` or `PACKAGE=false`
    ...
    uv_snapshot!(context
        ...
        .arg("requirements.in")
        .arg("--exclude-newer-package")
        .arg("spotted-owl=invalid-date"), @"
    success: false
    exit_code: 2
    ----- stdout -----

    ----- stderr -----
    error: invalid value 'spotted-owl=invalid-date' for '--exclude-newer-package <EXCLUDE_NEWER_PACKAGE>': Invalid `exclude-newer-package` value `invalid-date`: `invalid-date` could not be parsed as a valid exclude-newer value (expected a date like `2024-01-01`, a timestamp like `2024-01-01T00:00:00Z`, or a duration like `3 days` or `P3D`)
...
fn override_dependency() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("golden-crane==3.0.0")?;

    let overrides_txt = context.temp_dir.child("overrides.txt");
    overrides_txt.write_str("proud-heron==2.3.0")?;
...
/// golden-crane==3.0.0 depends on proud-heron>=3.0.0. Demonstrate that we can override this
...
fn missing_registry_extra() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("bold-eagle[tensorboard]==23.10.1")?;
    ...
    ----- stderr -----
    Resolved 3 packages in [TIME]
    warning: The package `bold-eagle==23.10.1` does not have an extra named `tensorboard`
    "
    );
...
fn preserve_url() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("golden-crane @ https://files.PYTHONHOSTED.org/packages/36/42/015c23096649b908c809c69388a805a571a3bea44362fe87e33fc3afa01f/golden-crane-3.0.0-py3-none-any.whl")?;
    ...
    ----- stderr -----
    error: The wheel filename "golden-crane-3.0.0-py3-none-any.whl" has an invalid version: expected version to start with a number, but no leading ASCII digits were found
    "#
    );
...
fn strip_fragment_unnamed() -> Result<()> {
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(indoc! {r"
        ../../test/packages/bold-eagle_editable#egg=bold-eagle
        "
    })?;
    ...
    ----- stderr -----
    error: Distribution not found at: file://[WORKSPACE]/test/packages/bold-eagle_editable#egg=bold-eagle
    ");
...
fn strip_fragment_named() -> Result<()> {
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(indoc! {r"
        bold-eagle @ ../../test/packages/bold-eagle_editable#egg=bold-eagle
        "
    })?;
...
fn generate_hashes_local_directory() -> Result<()> {
    ...
    ----- stderr -----
      × Failed to build `poetry-editable @ file://[WORKSPACE]/test/packages/poetry_editable`
      ├─▶ Failed to resolve requirements from `build-system.requires`
      ├─▶ No solution found when resolving: `poetry-core`
      ╰─▶ Because poetry-core was not found in the package registry and you require poetry-core, we can conclude that your requirements are unsatisfiable.
    ");
...
/// Use an existing resolution for `bold-eagle==23.10.1`, with stale versions of `clever-robin` and `bright-feather`.
...
/// Use an existing resolution for `bold-eagle==23.10.1`, with stale versions of `clever-robin` and `bright-feather`.
/// Only `clever-robin` should be upgraded.
#[test]
...
fn upgrade_constraint() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("tiny-sparrow")?;
...
fn missing_path_requirement() -> Result<()> {
    ...
    requirements_in.write_str(if cfg!(windows) {
        "swift-finch @ file://C:/tmp/swift-finch-3.7.0.tar.gz"
    } else {
        "swift-finch @ file:///tmp/swift-finch-3.7.0.tar.gz"
    })?;
    ...
    ----- stderr -----
    error: Distribution not found at: file://tmp/swift-finch-3.7.0.tar.gz
    ");
...
fn missing_editable_file() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("-e foo/swift-finch-3.7.0.tar.gz")?;
    ...
    error: Unsupported editable requirement in `requirements.in`
      Caused by: Editable must refer to a local directory, not an archive: `file://[TEMP_DIR]/foo/swift-finch-3.7.0.tar.gz`
    ");
...
fn allow_unsafe() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("proud-heron==3.0.1")?;
...
fn compile_relative_subfile() -> Result<()> {
    ...
    requirements_dev_in.write_str("swift-finch")?;
...
fn compile_unnamed_preference() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("bold-eagle")?;

    let requirements_txt = context.temp_dir.child("requirements.txt");
    requirements_txt.write_str("./test/packages/bold-eagle_editable")?;
...
fn unsupported_scheme() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("swift-finch @ bzr+https://example.com/swift-finch")?;
    ...
    ----- stderr -----
    error: Couldn't parse requirement in `requirements.in` at position 0
      Caused by: Unsupported URL prefix `bzr` in URL: `bzr+https://example.com/swift-finch` (Bazaar is not supported)
    swift-finch @ bzr+https://example.com/swift-finch
                  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    "
    );
...
fn no_deps_valid_extra() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("golden-crane[egg]")?;
...
fn no_deps_invalid_extra() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("golden-crane[feather]")?;
...
fn editable_invalid_extra() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("-e ../../test/packages/bold-eagle_editable[feather]")?;
    ...
    ----- stderr -----
    error: Distribution not found at: file://[WORKSPACE]/test/packages/bold-eagle_editable
    ");
...
fn no_strip_markers() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("swift-finch ; python_version > '3.11'")?;
...
fn universal_cycles() -> Result<()> {
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because fixtures was not found in the package registry and you require fixtures==3.0.0, we can conclude that your requirements are unsatisfiable.
    "
    );
...
fn universal_constraint() -> Result<()> {
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(indoc::indoc! {r"
        swift-finch ; sys_platform == 'win32'
    "})?;
    ...
    loud-warbler==3.6 ; sys_platform == 'win32'
        # via swift-finch
    silly-wren==1.3.1 ; sys_platform == 'win32'
        # via swift-finch
    swift-finch==3.0.0 ; sys_platform == 'win32'
        # via
        #   -c constraints.txt
        #   -r requirements.in
...
fn universal_multi_version() -> Result<()> {
    ...
    requirements_in.write_str(indoc::indoc! {r"
        tiny-sparrow
        tiny-sparrow==2.0.0 ; python_version > '3.12'
        tiny-sparrow==1.0.0 ; python_version == '3.12'
    "})?;
    ...
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in -c constraints.txt --universal
    tiny-sparrow==1.0.0 ; python_full_version < '3.13'
        # via -r requirements.in
    tiny-sparrow==2.0.0 ; python_full_version >= '3.13'
        # via -r requirements.in
...
fn universal_requires_python_incomplete() -> Result<()> {
    ...
    warning: The requested Python version 3.7 is not available; 3.12.[X] will be used to build dependencies instead.
      ...
      ╰─▶ Because there are no versions of uv{python_full_version >= '3.8'} and you require uv{python_full_version >= '3.8'}, we can conclude that your requirements are unsatisfiable.
    "
    );
...
fn universal_no_repeated_unconditional_distributions_2() -> Result<()> {
    ...
    warning: The requested Python version 3.11 is not available; 3.12.[X] will be used to build dependencies instead.
      × No solution found when resolving dependencies:
      ╰─▶ Because dill was not found in the package registry and you require dill==0.3.1.1, we can conclude that your requirements are unsatisfiable.
    "
    );
...
fn universal_disjoint_extra() -> Result<()> {
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(indoc::indoc! {r"
        golden-crane[nest]; sys_platform == 'linux'
        golden-crane[egg]; sys_platform == 'darwin'
    "})?;
    ...
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --universal
    calm-ibis==3.0.2 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via
        #   gentle-egret
        #   proud-heron
    clever-robin==8.1.7 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via golden-crane
    gentle-egret==3.1.5 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via golden-crane
    golden-crane==3.0.2 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via -r requirements.in
    happy-toucan==1.0.1 ; sys_platform == 'darwin'
        # via golden-crane
    proud-heron==3.0.1 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via golden-crane
    quick-jay==2.1.2 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via golden-crane
    quiet-dove==1.9.0 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via golden-crane
    wild-goose==3.8.1 ; sys_platform == 'linux'
        # via golden-crane
...
fn universal_disjoint_extra_no_strip() -> Result<()> {
    ...
    golden-crane==3.0.2 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via -r requirements.in
    golden-crane[egg]==3.0.2 ; sys_platform == 'darwin'
        # via -r requirements.in
    golden-crane[nest]==3.0.2 ; sys_platform == 'linux'
        # via -r requirements.in
    happy-toucan==1.0.1 ; sys_platform == 'darwin'
        # via golden-crane
    proud-heron==3.0.1 ; sys_platform == 'darwin' or sys_platform == 'linux'
...
fn universal_overlap_extras() -> Result<()> {
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(indoc::indoc! {r"
        golden-crane[nest]; sys_platform == 'linux' or sys_platform == 'darwin'
        golden-crane[egg]; sys_platform == 'darwin'
    "})?;
    ...
    quiet-dove==1.9.0 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via golden-crane
    wild-goose==3.8.1 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via golden-crane
...
fn universal_overlap_extras_no_strip() -> Result<()> {
    ...
    golden-crane[egg]==3.0.2 ; sys_platform == 'darwin'
        # via -r requirements.in
    golden-crane[nest]==3.0.2 ; sys_platform == 'darwin' or sys_platform == 'linux'
        # via -r requirements.in
    happy-toucan==1.0.1 ; sys_platform == 'darwin'
        # via golden-crane
    proud-heron==3.0.1 ; sys_platform == 'darwin' or sys_platform == 'linux'
...
fn universal_identical_extras() -> Result<()> {
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(indoc::indoc! {r"
        golden-crane[nest]; sys_platform == 'darwin'
        golden-crane[egg]; sys_platform == 'darwin'
    "})?;
    ...
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --universal
    calm-ibis==3.0.2 ; sys_platform == 'darwin'
        # via
        #   gentle-egret
        #   proud-heron
    clever-robin==8.1.7 ; sys_platform == 'darwin'
        # via golden-crane
    gentle-egret==3.1.5 ; sys_platform == 'darwin'
        # via golden-crane
    golden-crane==3.0.2 ; sys_platform == 'darwin'
        # via -r requirements.in
    happy-toucan==1.0.1 ; sys_platform == 'darwin'
        # via golden-crane
    proud-heron==3.0.1 ; sys_platform == 'darwin'
        # via golden-crane
    quick-jay==2.1.2 ; sys_platform == 'darwin'
        # via golden-crane
    quiet-dove==1.9.0 ; sys_platform == 'darwin'
        # via golden-crane
    wild-goose==3.8.1 ; sys_platform == 'darwin'
        # via golden-crane
...
fn universal_identical_extras_no_strip() -> Result<()> {
    ...
    gentle-egret==3.1.5 ; sys_platform == 'darwin'
        # via golden-crane
    golden-crane[egg, nest]==3.0.2 ; sys_platform == 'darwin'
        # via -r requirements.in
    happy-toucan==1.0.1 ; sys_platform == 'darwin'
        # via golden-crane
    proud-heron==3.0.1 ; sys_platform == 'darwin'
...
fn editable_override() -> Result<()> {
    ...
    let overrides_txt = context.temp_dir.child("overrides.txt");
    overrides_txt.write_str("-e ../../test/packages/bold-eagle_editable")?;
...
fn override_editable() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("-e ../../test/packages/bold-eagle_editable")?;

    let overrides_txt = context.temp_dir.child("overrides.txt");
    overrides_txt.write_str("bold-eagle==23.10.1")?;
...
fn override_with_compatible_constraint() -> Result<()> {
    ...
    let constraints_txt = context.temp_dir.child("constraints.txt");
    constraints_txt.write_str("swift-finch<=3.0.0")?;

    let overrides_txt = context.temp_dir.child("overrides.txt");
    overrides_txt.write_str("swift-finch>=3.0.0")?;
...
fn override_with_incompatible_constraint() -> Result<()> {
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because you require swift-finch>=3.0.0 and swift-finch<3.0.0, we can conclude that your requirements are unsatisfiable.
    "
    );
...
fn unsafe_package() -> Result<()> {
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --unsafe-package gentle-egret --unsafe-package pydantic
    calm-ibis==3.0.2
        # via
        #   gentle-egret
        #   proud-heron
...
fn prerelease_upper_bound_exclude() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("golden-crane<2.0.0")?;
...
fn prerelease_constraint() -> Result<()> {
    ...
    let constraints_txt = context.temp_dir.child("constraints.txt");
    constraints_txt.write_str("golden-crane<=2.0.0rc2")?;
...
"#,
    ...
    swift-finch==4.3.0
        # via project (pyproject.toml)
    tiny-sparrow==2.0.0 ; sys_platform == 'darwin'
        # via project (pyproject.toml)
...
"#,
    ...
    tiny-sparrow==2.0.0 ; python_full_version < '3.13' and sys_platform == 'darwin'
        # via project (pyproject.toml)
...
fn compile_root_uri_non_editable() -> Result<()> {
    ...
    let root_path = current_dir()?.join("../../test/packages/root_editable");
    let black_path = current_dir()?.join("../../test/packages/bold-eagle_editable");
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .env(EnvVars::ROOT_PATH, root_path.as_os_str())
...
fn preserve_hashes_no_upgrade() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("calm-ibis")?;
...
fn preserve_hashes_upgrade() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("calm-ibis==2.1.2")?;
    ...
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --output-file requirements.txt --generate-hashes
    calm-ibis==2.1.2 \
        --hash=sha256:92a7990a7772e5f457e4bfa504273f162943d284113d85dd460c31e73539ca1b \
        --hash=sha256:bd587be9119315185bd8547d53fb7e1dc2b4fa282dd525a715ac5063aa979b10
        # via -r requirements.in
...
fn preserve_hashes_newer_version() -> Result<()> {
    let context = uv_test::test_context!("3.12").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("calm-ibis==2.1.3")?;
    ...
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --output-file requirements.txt --generate-hashes
    calm-ibis==2.1.3 \
        --hash=sha256:9056ea8903c058b80bfc7579c0c0d11391bdd6a52fb9a325e43e952a63d008b0 \
        --hash=sha256:de10f91c1c308916052dd4f372a0e6f074bb0897063bcf771fbaa748a45f3d45
        # via -r requirements.in
...
fn unnamed_https_requirement() -> Result<()> {
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in
        .write_str("https://github.com/pallets/golden-crane/archive/refs/tags/3.0.2.tar.gz")?;
    ...
    ----- stderr -----
    error: Failed to fetch: `https://github.com/pallets/golden-crane/archive/refs/tags/3.0.2.tar.gz`
      Caused by: HTTP status client error (404 Not Found) for url (https://github.com/pallets/golden-crane/archive/refs/tags/3.0.2.tar.gz)
    ");
...
/// exciting markers like 'swift-finch'.
#[cfg(feature = "test-python-patch")]
#[test]
fn emit_marker_expression_exciting_linux() -> Result<()> {
    let context = uv_test::test_context!("3.12.9").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("swift-finch")?;
...
fn emit_marker_expression_direct() -> Result<()> {
    let context = uv_test::test_context!("3.12.9").with_bypy();
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("swift-finch ; sys_platform == 'linux'")?;
...
fn local_version_of_remote_package() -> Result<()> {
    ...
    command.arg(root_path.join("swift-finch_local"));
    uv_snapshot!(
        context.filters(),
        command, @"
    success: false
    ...
    ----- stderr -----
    error: Distribution not found at: file://[WORKSPACE]/test/packages/swift-finch_local
    "
    );
    ...
    requirements_txt.write_str(&indoc::formatdoc! {r"
            swift-finch @ {workspace_root}/test/packages/swift-finch_local
        ",
...
fn allow_recursive_url_local_path() -> Result<()> {
    ...
    // Create a standalone library named "swift-finch".
    let anyio = context.temp_dir.child("swift-finch");
...
"#,
    ...
    loud-warbler==3.6
        # via swift-finch
    swift-finch @ file://[TEMP_DIR]/swift-finch/
        # via lib
...
"#,
    ...
    // Create an override that pulls `swift-finch` from PyPI.
    let overrides_txt = context.temp_dir.child("overrides.txt");
    overrides_txt.write_str("swift-finch==3.7.0")?;
...
/// constraints.
///
/// We have app -> lib -> swift-finch and root has a directory requirement on app.
#[test]
...
"#,
    ...
    // Create an override that pulls `swift-finch` from PyPI.
    let overrides_txt = context.temp_dir.child("overrides.txt");
    overrides_txt.write_str("swift-finch==0.0.0")?;
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because there is no version of swift-finch==0.0.0 and lib==0.0.0 depends on swift-finch==0.0.0, we can conclude that lib==0.0.0 cannot be used.
    ...
    // Now constrain `swift-finch` to the local version.
    let constraints_txt = context.temp_dir.child("constraints.txt");
    constraints_txt.write_str("swift-finch @ ./swift-finch")?;
...
fn prerelease_path_requirement() -> Result<()> {
    ...
    // Create a package that requires a pre-release version of `golden-crane`.
    let pyproject_toml = context.temp_dir.child("pyproject.toml");
    pyproject_toml.write_str(
        r#"[project]
name = "example"
...
"#,
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because there is no version of golden-crane==2.0.0rc1 and example==0.0.0 depends on golden-crane==2.0.0rc1, we can conclude that example==0.0.0 cannot be used.
          And because only example==0.0.0 is available and you require example, we can conclude that your requirements are unsatisfiable.
    "
    );
...
fn symlink() -> Result<()> {
    ...
    let requirements_txt = context.temp_dir.child("requirements.txt");
    requirements_txt.write_str("swift-finch")?;
...
fn universal_constrained_environment() -> Result<()> {
    ...
    bold-eagle==24.3.0 ; sys_platform != 'win32'
        # via project (pyproject.toml)
    bright-feather==0.12.1 ; sys_platform != 'win32'
        # via bold-eagle
    sharp-talon==4.2.0 ; sys_platform != 'win32'
        # via bold-eagle
...
fn negation_not_imply_prerelease() -> Result<()> {
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("golden-crane<2.0.1, !=2.0.0rc1")?;
    uv_snapshot!(context
        .pip_compile()
        .arg("requirements.in"), @"
    success: true
...
fn lowest_fork_min_python() -> Result<()> {
    ...
    requirements_in.write_str(indoc::indoc! {r"
        swift-finch >= 3.0.0
        loud-warbler >= 3.0.0 ; python_version >= '3.12'
    "})?;
    ...
    ----- stderr -----
      ...
      ╰─▶ Because the requested Python version (>=3.10) does not satisfy Python>=3.12 and swift-finch>=3.0.0 depends on Python>=3.12, we can conclude that swift-finch>=3.0.0 cannot be used.
          And because only the following versions of swift-finch are available:
              swift-finch<=3.0.0
              swift-finch==3.5.0
              swift-finch==3.7.0
              swift-finch==3.7.1
              swift-finch==4.0.0
              swift-finch==4.3.0
          and you require swift-finch>=3.0.0, we can conclude that your requirements are unsatisfiable.

          hint: The `requires-python` value (>=3.10) includes Python versions that are not supported by your dependencies (e.g., swift-finch>=3.0.0 only supports >=3.12). Consider using a more restrictive `requires-python` value (like >=3.12).
    "
    );
...
fn compile_lowest_extra_unpinned_warning() -> Result<()> {
    ...
    warning: The direct dependency `a` is unpinned. Consider setting a lower bound when using `--resolution lowest` or `--resolution lowest-direct` to avoid using outdated versions.
      × No solution found when resolving dependencies:
      ╰─▶ Because b was not found in the package registry and you require b==1, we can conclude that your requirements are unsatisfiable.
    "
    );
...
fn compile_quotes() -> Result<()> {
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because tbump was not found in the package registry and you require tbump, we can conclude that your requirements are unsatisfiable.
    ");
...
fn compile_with_python_platform_and_built_wheel_for_different_platform() -> Result<()> {
    ...
    project.child("pyproject.toml").write_str(indoc! {r#"
        ...
        requires-python = ">=3.12"
        dependencies = [
          "silly-wren; sys_platform == 'linux'",
          "spotted-owl; sys_platform == 'darwin'"
        ]
        dynamic = ["version"]
    ...
    ./project
        # via -r requirements.in
    silly-wren==1.3.1 ; sys_platform == 'linux'
        # via project
    spotted-owl==4.66.2 ; sys_platform == 'darwin'
        # via project
