...
fn invalid_platform() -> Result<()> {
    ...
    ----- stderr -----
      ...
      ╰─▶ Because only the following versions of open3d are available:
              ...
              open3d==0.17.0
              open3d==0.18.0
          and open3d<=0.15.2 has no wheels with a matching Python version tag (e.g., `cp310`), we can conclude that open3d<=0.15.2 cannot be used.
          ...
          hint: You require CPython 3.10 (`cp310`), but we only found wheels for `open3d` (v0.15.2) with the following Python version tags: `cp36m`, `cp37m`, `cp38`, `cp39`
