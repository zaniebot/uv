...
/// Use an existing resolution for `black==23.10.1`, with stale versions of `click` and `pathspec`.
/// With `--upgrade --upgrade-strategy direct`, `black` is already at the requested
...
#[test]
fn upgrade_strategy_direct() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("black==23.10.1")?;
    ...
    // With `--upgrade --upgrade-strategy direct`, only the direct dependency `black`
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --output-file requirements.txt --upgrade-strategy direct
    black==23.10.1
        # via -r requirements.in
    click==8.1.2
        # via black
...
/// With `--upgrade --upgrade-strategy direct` and an unpinned direct dependency,
...
#[test]
fn upgrade_strategy_direct_unpinned() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    ...
    // With `--upgrade --upgrade-strategy direct`, `black` (unpinned) gets upgraded to
...
/// With `--upgrade --upgrade-strategy transitive` (the default), both direct and transitive
/// dependencies should be upgraded. This test confirms that `--upgrade-strategy transitive`
/// behaves identically to `--upgrade` alone.
#[test]
fn upgrade_strategy_transitive() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("black==23.10.1")?;
    ...
    // With `--upgrade --upgrade-strategy transitive`, all packages are upgraded (same as `--upgrade`).
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .arg("--output-file")
        .arg("requirements.txt")
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --output-file requirements.txt --upgrade-strategy transitive
    black==23.10.1
        # via -r requirements.in
    click==8.1.7
        # via black
