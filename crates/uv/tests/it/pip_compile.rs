...
/// Test that `--hashes compatible` generates platform-specific hashes only.
#[test]
fn generate_hashes_compatible_mode() -> Result<()> {
    let context = TestContext::new("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("numpy==1.26.4")?;

    // Test with --hashes all (should show multiple hashes for different platforms)
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --generate-hashes --hashes all
    numpy==1.26.4 \
    ...
    // Test with --hashes compatible (should show only platform-specific hashes)
    uv_snapshot!(context.pip_compile()
        .arg("requirements.in")
        .arg("--generate-hashes")
        .arg("--hashes")
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --generate-hashes --hashes compatible
    numpy==1.26.4 \
        --hash=sha256:b3ce300f3644fb06443ee2222c2201dd3a89ea6040541412b8fa189341847218 \
        --hash=sha256:03a8c78d01d9781b28a6989f6fa1bb2c4f2d51201cf99d3dd875df6fbd96b23b
        # via -r requirements.in
...
/// Test that `--hashes compatible` works with multiple packages.
#[test]
fn generate_hashes_compatible_mode_multiple_packages() -> Result<()> {
    let context = TestContext::new("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("numpy==1.26.4\npandas==2.1.4")?;
