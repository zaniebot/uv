...
fn clean_background() -> Result<()> {
    ...
    // Wait briefly for background deletion to complete, then verify no temp directories remain
    std::thread::sleep(std::time::Duration::from_secs(2));

    // Check that no temp directories with our prefix remain in the parent directory
    ...
    assert!(
        temp_dirs.is_empty(),
        "Expected background daemon to have cleaned up temp directories, found: {temp_dirs:?}"
    );
