...
#[derive(Debug, Clone)]
struct ResolvedPythonRequest {
    source: PythonRequestSource,
    request: Option<PythonRequest>,
}

impl ResolvedPythonRequest {
    /// Resolve a Python request from the provided options.
    async fn from_options(
        explicit_request: Option<PythonRequest>,
        project_dir: &Path,
        workspace_root: Option<&Path>,
        no_config: bool,
        script_requires_python: Option<&RequiresPython>,
        workspace_requires_python: Option<&RequiresPython>,
    ) -> Result<Option<Self>, ProjectError> {
        if let Some(request) = explicit_request {
            return Ok(Some(Self {
                source: PythonRequestSource::UserRequest,
                request: Some(request),
            }));
        }

        let Some(file) = PythonVersionFile::discover(
            project_dir,
        ...
        .await?
        .filter(|file| match (file.version(), script_requires_python) {
        ...
        .filter(|file| {
            ...
            match (file.version(), workspace_requires_python) {
                (Some(request), Some(requires_python)) => {
                    python_request_compatible_with_requires_python(request, requires_python)
                }
                _ => true,
        ...
        Ok(Some(Self {
            source: PythonRequestSource::DotPythonVersion(file.clone()),
            request: file.version().cloned(),
        }))
    }
}
...
impl ResolvedPython {
    async fn from_request(
        ...
        groups: &DependencyGroupsWithDefaults,
        project_dir: &Path,
        script_requires_python_specifiers: Option<&VersionSpecifiers>,
        no_config: bool,
    ) -> Result<Self, ProjectError> {
        let script_requires_python =
            script_requires_python_specifiers.map(RequiresPython::from_specifiers);
        ...
        let workspace_root = workspace.map(Workspace::install_path).map(PathBuf::as_path);

        let (source, python_request) = if let Some(resolved) = ResolvedPythonRequest::from_options(
            explicit_request,
            project_dir,
            workspace_root,
            no_config,
            script_requires_python.as_ref(),
            workspace_requires_python.as_ref(),
        )
        .await?
        {
            (resolved.source, resolved.request)
        } else if let Some(requires_python) = script_requires_python.as_ref() {
            (
                PythonRequestSource::RequiresPython,
            ...
            (
                PythonRequestSource::RequiresPython,
                workspace_requires_python.as_ref().map(|requires_python| {
...
impl ScriptPython {
    ...
    pub(crate) async fn from_request(
        ...
        let ResolvedPython {
            ...
            &DependencyGroupsWithDefaults::none(),
            project_dir,
            script.metadata().requires_python.as_ref(),
            no_config,
        )
        .await?;
