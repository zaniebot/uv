...
use uv_bin_install::install_compatible_uv;
...
/// Handle version mismatch by attempting to auto-install and re-exec with compatible version.
async fn handle_version_mismatch(
    ...
    eprintln!(
        "Current uv version `{}` does not match required version `{}`. Attempting to find compatible version...",
        package_version, required_version
    );

    // Try to install a compatible version
    match install_compatible_uv(required_version.specifiers(), client, cache, None).await {
        Ok(compatible_uv_path) => {
            eprintln!("Compatible uv found. Re-executing with compatible version...");

            // Re-execute with the compatible version
            let mut cmd = Command::new(&compatible_uv_path);

            // Skip the first argument (original binary name) and pass the rest
            ...
            let status = cmd
                .status()
                .context("Failed to execute compatible uv version")?;
        ...
        Err(err) => {
            ...
            let hint = {
                ...
                if let Some(singleton) = ranges.as_singleton() {
                    // E.g., `==1.0.0`
                    format!(
                        ". Alternatively, update `uv` by running `{}`.",
            ...
            Err(anyhow::anyhow!(
                "Required uv version `{required_version}` does not match the running version `{package_version}`{hint}. Auto-install failed: {err}",
            ))
        }
    }
}
...
#[instrument(skip_all)]
async fn run(mut cli: Cli, original_args: Vec<OsString>) -> Result<ExitStatus> {
    ...
    // Note: Version check moved after cache creation to allow auto-install
    ...
    // Check for required version and potentially auto-install compatible version.
    if let Some(required_version) = globals.required_version.as_ref() {
        ...
        if !required_version.contains(&package_version) {
            // Check if install-required-version preview feature is enabled
            if globals
                .preview
                .is_enabled(uv_configuration::PreviewFeatures::INSTALL_REQUIRED_VERSION)
            {
                // Create a basic client for GitHub API calls
                let client = BaseClientBuilder::new().build();

                return handle_version_mismatch(
                    required_version,
                    &package_version,
                    &original_args,
                    &cache,
                ...
                // Fall back to the original error with hint about the preview feature
                return Err(anyhow::anyhow!(
                    "Required uv version `{required_version}` does not match the running version `{package_version}`. To enable auto-install, use `--preview-feature install-required-version`."
                ));
            }
        }
    }
...
{
    // Capture original args for potential re-execution
    ...
    let cli = match Cli::try_parse_from(&original_args) {
        Ok(cli) => cli,
        Err(mut err) => {
    ...
    let main2 = move || {
        ...
        let result = runtime.block_on(Box::pin(run(cli, original_args.clone())));
