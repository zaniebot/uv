...
// glibc version is taken from std/sys/unix/libc.rs
fn detect_glibc_version_from_ldd() -> Result<Os, Error> {
...
fn glibc_ldd_output_to_version(kind: &str, output: &[u8]) -> Result<Os, Error> {
    ...
    let Some(libc) = parse_glibc_version(version) else {
...
fn detect_linux_libc_from_ld_symlink(path: &Path) -> Result<Os, Error> {
    static RE: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"^ld-([0-9]{1,3})\.([0-9]{1,3})\.so$").unwrap());

    let target = fs::read_link(path)
        .map_err(|err| anyhow!(format!("failed to read {path:?} as a symbolic link: {err}",)))?;
...
/// ```
fn detect_musl_version(ld_path: impl AsRef<Path>) -> Result<Os, Error> {
    let ld_path = ld_path.as_ref();
    let output = Command::new(ld_path)
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .output()
        .map_err(|err| anyhow!(format!("failed to execute `{ld_path:?}` for musl: {err}")))?;
...
fn musl_ld_output_to_version(kind: &str, output: &[u8]) -> Result<Os, Error> {
    static RE: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"Version ([0-9]{1,4})\.([0-9]{1,4})").unwrap());

    let output = std::str::from_utf8(output)
        .map_err(|err| anyhow!(format!("failed to parse {kind} as UTF-8: {err}")))?;
...
fn find_ld_path() -> Result<PathBuf, Error> {
...
fn find_ld_path_at(path: impl AsRef<Path>) -> Result<PathBuf, Error> {
